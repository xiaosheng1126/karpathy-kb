...
模板文件使用 Markdown + Jinja2 风格占位符。模板只负责结构，不包含判断逻辑。判断逻辑由 Output Layer 的 LLM 调用完成。
...
- [ ] Knowledge Compiler 基础实现：多 raw → wiki topic
- [ ] 往回织：新 raw 进入时检查 `wiki_targets`，提示相关 wiki 更新
- [ ] raw → wiki 关联索引（支撑 Compiler 高效路由）

### v3 目标：多角色支持

- [ ] Role Profile 加载器（从 `config/roles/` 动态读取）
- [ ] 新增第二个角色（`product_builder` 或 `writer`）
- [ ] Output Template 渲染引擎（支持 Jinja2 风格模板）

### v4 目标：知识生命周期管理

- [ ] 自动 aging 检测（扫描 `valid_until` 临近的 raw 和 wiki 判断）
- [ ] 周报中展示"即将过期的判断"模块
- [ ] 用户确认 deprecated 的交互流
