...
Ingestion Adapter（kb.py raw）
  ↓  [RawNote v1]
Raw Layer（raw_dir/*.md，默认 Obsidian Vault/00_Inbox）
  ↓  [RawQuery]
...
新角色的加入只需新增一个 Role Profile 数据文件和对应模板/指令文件，不改核心代码。
...
| `%%WEEKLY_INSTRUCTIONS%%` | Role Profile 的 `instructions_file` 全文，默认 `prompts/weekly.md` |
...
### 示例：product_builder 角色（已实现）

添加"产品思考者"角色只需新增角色配置、输出模板和角色指令文件，零核心代码变更：
...
**`prompts/weekly_product.md`**：定义产品视角的判断逻辑，避免复用技术周报指令。

验证方式：`python3 scripts/kb.py weekly --role product_builder --no-cache`
...
- [x] 新增第二个角色：`product_builder`，包含 Role Profile、Output Template 和产品周报指令
